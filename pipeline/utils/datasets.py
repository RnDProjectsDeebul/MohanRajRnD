...
def get_notMNIST(root="../../"):
    input_size = 28
    num_classes = 10
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.4254,), (0.4586,))])
...
def get_FastFashionMNIST(root="../../"):
    input_size = 28
    num_classes = 10
    dataset = FastFashionMNIST(root + "data/", train=True, download=True)
    test_dataset = FastFashionMNIST(root + "data/", train=False, download=True)
    return input_size, num_classes, dataset, test_dataset
